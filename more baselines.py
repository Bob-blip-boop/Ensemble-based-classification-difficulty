...
def density_sampling(X_pool,n_instances):
    euclidean_density = information_density(X_pool, 'euclidean')
    sorted_density_ind = np.argsort(euclidean_density)
    diff_ind = sorted_density_ind[:n_instances]
    return diff_ind
...
def mlp(X_train, X_test, y_train, global_rand_seed):
    model = MLPClassifier(random_state = global_rand_seed)
    model.fit(X_train,y_train)
    predicted_label = model.predict(X_test)
...
def get_disagreement(X_train, X_test, y_train, global_rand_seed):
    ...
    frequency = np.transpose([dt_predicted_label,mlp_predicted_label,knn_predicted_label,nb_predicted_label,rf_predicted_label])
    
    disagreement = []
    for i in range(len(frequency)):
        d = len(np.unique(frequency[i])) / 5
    ...
    return disagreement, clf_scores

def get_disagreement_3(X_train, X_test, y_train, global_rand_seed):
    dt_predicted_label, dt_score = decisiontree(X_train, X_test, y_train, global_rand_seed)
    knn_predicted_label, knn_score  = KNN(X_train, X_test, y_train)
    nb_predicted_label, nb_score  = NB(X_train, X_test, y_train)     


    frequency = np.transpose([dt_predicted_label,knn_predicted_label,nb_predicted_label])
    
    disagreement = []
    for i in range(len(frequency)):
        d = len(np.unique(frequency[i])) / 3
        disagreement.append(d)
    
    clf_scores = [dt_score, knn_score, nb_score]
    clf_scores = [np.array(x) for x in clf_scores]
    clf_scores = [np.mean(k) for k in zip(*clf_scores)]
        
    return disagreement, clf_scores

def get_disagreement_2(X_train, X_test, y_train, global_rand_seed):
    dt_predicted_label, dt_score = decisiontree(X_train, X_test, y_train, global_rand_seed)
    knn_predicted_label, knn_score  = KNN(X_train, X_test, y_train)


    frequency = np.transpose([dt_predicted_label,knn_predicted_label])
    ...
    clf_scores = [dt_score, knn_score]
    clf_scores = [np.array(x) for x in clf_scores]
    clf_scores = [np.mean(k) for k in zip(*clf_scores)]
        
    return disagreement, clf_scores
    
def QBC_sampling(X_train, y_train, n_instances, X_pool, global_rand_seed):
    #disagreement, clf_scores = get_disagreement(X_train, X_pool, y_train, global_rand_seed)
    disagreement, clf_scores = get_disagreement_3(X_train, X_pool, y_train, global_rand_seed)
    #disagreement, clf_scores = get_disagreement_2(X_train, X_pool, y_train, global_rand_seed)
    clf_scores = [1 - i for i in clf_scores]
    sorted_disagreement_ind = np.argsort(clf_scores)
    diff_ind = sorted_disagreement_ind[-n_instances:]
    return diff_ind
...
#for data in range(1,2):
    ...
    batch_size = 0.01              #In Percentage .01 = 1%   Default: 1%
    diff_size = 1                  #In Percentage 1 = 100%
    initial_train_size = 0.01   #In Percentage 0.001 = .1%  Default: 5%
    ...
    for global_rand_seed in seeds:
        ...
        for i in range(n, len(X_pool), n):
            ...
            #margin Sampling 
            start = timeit.default_timer()
            uncer_idx = margin_sampling(uncer_learner, uncer_X_pool, n_instances = n)
            stop = timeit.default_timer()
            ...
            #QBC Sampling 
            start = timeit.default_timer()
            clf_cnt_indx = QBC_sampling(clf_cnt_X_train, clf_cnt_y_train, n, clf_cnt_X_pool, global_rand_seed)
            #print(clf_cnt_X_pool)
    ...
    print("margin_INST =", uncer_inst_avg, uncer_inst_std)
    print("qbc_INST=", clf_cnt_inst_avg, clf_cnt_inst_std)
    ...
    plt.plot(x_axis, reap_uncer_acc_lst_avg, label = "Margin")
    plt.plot(x_axis, reap_clf_cnt_acc_lst_avg, label = "QBC")
    ...
    print("Dataset:", name, "Batch Size", n, "Diff Size", diff_perc, "Initial Train", initial_train_size, "Mode", mode, "Base Learners", n_estimators)
    print("random_ALC =", alc_lst[0], rand_std)
    print("margin_ALC =", alc_lst[1], uncer_std)
    print("qbc_ALC =", alc_lst[2], clf_cnt_std)
    print("")
    sys.stdout.close()
    sys.stdout=stdoutOrigin
...
print("margin_"+ diff_size +"_ALC = ", uncer_auc_lst)
print("qbc_"+ diff_size +"_ALC = ", clf_cnt_auc_lst)
print("")   
